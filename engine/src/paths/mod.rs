...
//!     // Convert from `Path`
//!     assert_eq!(
//!         Path::new(r"foo\bar\piyo.txt").as_posix().unwrap(),
//!         "foo/bar/piyo.txt",
//!     );
//!
...
//!     assert_eq!(p.as_posix().unwrap(), "foo/bar/piyo.txt");
//!
...
//!     // Convert from `Path`
//!     assert_eq!(
//!         Path::new("foo/bar/piyo.txt").as_posix().unwrap(),
//!         "foo/bar/piyo.txt",
//!     );
//!
//!     // Convert to/from PathBuf
...
pub trait PathExt {
    ...
    ///
    /// assert_eq!(s.as_posix(), Some(Cow::Borrowed("foo/bar/piyo.txt")));
    /// ```
    fn as_posix(&self) -> Option<Cow<'_, str>>;
    /// Convert the file path into slash path as UTF-8 string. This method is similar to
    ...
    /// let s = Path::new("foo/bar/piyo.txt");
    ///
    /// assert_eq!(s.as_posix_lossy(), "foo/bar/piyo.txt");
    /// ```
    fn as_posix_lossy(&self) -> Cow<'_, str>;
}
...
///
/// assert_eq!(
///     PathBuf::from_slash("foo/bar/piyo.txt").as_posix().unwrap(),
///     "foo/bar/piyo.txt",
/// );
/// ```
...
///
/// assert_eq!(
///     Cow::from_slash("foo/bar/piyo.txt").as_posix_lossy(),
///     "foo/bar/piyo.txt",
/// );
/// ```
pub trait CowExt<'a> {
...
impl<'a> CowExt<'a> for Cow<'a, Path> {
    ...
    fn as_posix(&self) -> Option<Cow<'_, str>> {
        self.as_ref().as_posix()
    }

    fn as_posix_lossy(&self) -> Cow<'_, str> {
        self.as_ref().as_posix_lossy()
    }
}
